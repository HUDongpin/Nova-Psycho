...
GET `/api/audit` (admin) -> array of `{id,action,entity_id,created_at}` for the actor's region, newest first, `LIMIT 500`. The SQL selects only those four columns; the list does not include actor names or free-text payloads. Staff, parents, students and teachers receive `403 ROLE_DENIED`. Events are append-only and not editable through this API. The response must not invent clinical meaning.
